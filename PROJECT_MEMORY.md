...
on-orbit and serves the spacecraft On-Board Computer (OBC) over RS-422.

### Top-level commands (only two, no local storage)

| Command | Action |
|---|---|
| `CMD_CAPTURE_TO_OBC(camera, format)` | Capture one frame, send JPEG to OBC over RS-422 UART |
...
- **Full-res photo path**: 4K (or quad-tile 4K) JPEG → RS-422 → OBC → S-band downlink
...
| MIPI-DSI display (1280×720) | RS-422 → OBC → S-band | Live frame preview, debug visualization, no SD-card or RS-422 dump needed |
...
- **OBC ↔ P4**: RS-422 UART (full-duplex differential)
...
- **S-band downlink**: P4 doesn't touch it. P4 streams JPEG to OBC over
  RS-422; OBC handles S-band path.
...
| JPEG output | ~3–4 MB | until RS-422 finished |
| MI1602 frame | 38 KB | per IR shot |
| SSTV PCM buffer | 1.2 MB | 36 s × 16 kHz × 16-bit |
| RS-422 DMA buffers | 2 × 4 KB | continuous |
...
| RS-422 transfer of 3 MB | 30 s @ 921k | ~250 mW |
...
7. **RS-422 framed protocol** — likely CSP or StackFlow-style JSON
...
- [ ] **RS-422 protocol** (no OBC link yet)
...
- ❌ RS-422 protocol layer (not yet implemented)
...
- [ ] Implement RS-422 protocol over USB-CDC (treat USB CDC as the
      RS-422 link during dev) — frames go to a PC test app
...
- [ ] Custom CubeSat carrier: SC850SL FPC, MI1602 FPC, ES8388-class codec,
      RS-422 transceiver (MAX3491E), VHF radio interface, power rails
